...
2026-07-24 — Free/paid Nassau naming split (supersedes the naming half of the 2026-06-23 entry above): With a paid ‘Nassau Port Day Playbook’ now listed on Etsy at $5.99, the free web resource and the paid product could no longer share the ‘Playbook’ name. The owner split them: ‘Playbook’ is now reserved exclusively for the **paid** Etsy product, and the **free** web resource is renamed ‘Nassau Survival Card’ everywhere it is visible — the `/nassau-cheat-sheet` opt-in page, the `/nassau-survival-card` guide page, the planner email gate, and the post-unlock confirmation. No route paths changed.

**Kit-hosted surfaces, 2026-07-24.** The live delivery email (sequence ‘Nassau Plan Delivery’ 2807323, email 10009134) was updated via the Kit API: subject is now ‘Your Nassau plan + return target’, the body calls the free asset the Nassau Survival Card, and the primary button reads ‘Read the Nassau Survival Card →’. Triggers, delay, send days, tags, consent logic, destination URLs and every UTM parameter (including the historical `utm_content=playbook_primary`) were left untouched so existing attribution keeps reconciling.

**Kit form, 2026-07-24 — done, no residual.** The embedded form (id 9599500, UID `140b41e206`) was renamed to ‘Nassau Survival Card Form’ and its submit button changed to ‘Get the Free Survival Card’. This one was edited by the owner **manually in the Kit dashboard**: Kit’s v4 API exposes no form-mutation endpoint (the MCP surface is limited to listing forms and attaching subscribers), so form name and button text cannot be scripted. Verified two ways — the Kit API reports the new form name, and a cache-busted load of `shoreday.kit.com/140b41e206/index.js` renders ‘Get the Free Survival Card’ on the public `/nassau-cheat-sheet` page. Note for future checks: the embed script is HTTP-cached, so a browser that already loaded the page keeps showing the old button until its cache expires — always verify with a cache-busting request rather than a plain reload.
...
2026-06-23 — Nassau V1 timing policy (locked, owner-approved): ShoreDay's approved planning defaults are `terminalBufferMinutes = 45`, `defaultContingencyMinutes = 15`, `minimumUsableMinutes = 120` (in `data/ports/nassau.ts`, `configStatus: 'locked'`). These are ShoreDay **product-policy** planning defaults, not external facts and not guarantees: they are planning recommendations only; users must confirm the official ship-provided all-aboard time (the final authority); V1 does not monitor live schedule changes, traffic, weather, or port conditions; no itinerary or recommendation guarantees a return to the ship; and the 120-minute minimum-usable threshold is a ShoreDay conservative product threshold, not an externally verified universal rule. (Broader ~45–60-minute return practices in cruise guidance are retained only as contextual research, not as the locked configuration.)
